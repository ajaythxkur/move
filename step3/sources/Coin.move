module coin::test_coin{
    ...
    struct  CoinCapabilities has key{
        mint_cap: coin::MintCapability<TestCoin>, //if fixed supply, remove!!
    ...
    fun init_module(account: &signer){
        ...
        move_to(account, CoinCapabilities {mint_cap, burn_cap, freeze_cap}); //if fixed supply, destory mint_cap!!
    }
    ...
    public entry fun freeze_user(account: &signer, user: address) acquires CoinCapabilities{
        ...
        coin::freeze_coin_store<TestCoin>(user, freeze_cap);
    }
    ...
    public entry fun unfreeze_user(account: &signer, user: address) acquires CoinCapabilities{
        ...
        let freeze_cap = &borrow_global<CoinCapabilities>(@admin).freeze_cap;
        coin::unfreeze_coin_store<TestCoin>(user, freeze_cap);
    }
}